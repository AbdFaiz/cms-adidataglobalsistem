...
@section('title', 'Volt Laravel Dashboard - Profile')

@section('content')
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center py-4">
        <div>
            <a href="{{ route('users.index') }}" class="btn btn-gray-800 me-2">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input disabled name="name" class="form-control" id="name" type="text"
                                placeholder="Enter your name" required value="{{ old('name', $viewUser->name) }}">
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="username">Username</label>
                            <input disabled name="username" class="form-control" id="username" type="text"
                                placeholder="Enter your username" required
                                value="{{ old('username', $viewUser->username) }}">
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-12 mb-3">
                            ...
                            <input disabled name="email" class="form-control" id="email" type="email"
                                placeholder="name@company.com" value="{{ old('email', $viewUser->email) }}">
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-12 mb-3">
                            ...
                            <input disabled name="phone" class="form-control" id="phone" type="text"
                                placeholder="+12-345 678 910" value="{{ old('phone', $viewUser->phone) }}">
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <select disabled name="status" class="form-select" id="status" required>
                                <option value="" disabled
                                    {{ old('status', $viewUser->status ?? '') == '' ? 'selected' : '' }}>Choose</option>
                                <option value="active"
                                    {{ old('status', $viewUser->status ?? '') == 'active' ? 'selected' : '' }}>Active
                                </option>
                                <option value="pending"
                                    {{ old('status', $viewUser->status ?? '') == 'pending' ? 'selected' : '' }}>Pending
                                </option>
                                <option value="suspended"
                                    {{ old('status', $viewUser->status ?? '') == 'suspended' ? 'selected' : '' }}>Suspended
                                </option>
                                <option value="inactive"
                                    {{ old('status', $viewUser->status ?? '') == 'inactive' ? 'selected' : '' }}>Inactive
                                </option>
                            </select>
                        </div>
                        <div class="col-md-6 mb-3">
                ...
                <div class="col-12 mb-4">
                    <div class="card shadow border-0 text-center p-0">
                        <div class="profile-cover rounded-top"
                            style="background-image:url('{{ $viewUser->avatar ? asset('storage/' . $viewUser->avatar) : asset('storage/avatars/default-avatar.png') }}')">
                        </div>
                        <div class="card-body pb-5">
                            <img src="{{ $viewUser->avatar ? asset('storage/' . $viewUser->avatar) : asset('storage/avatars/default-avatar.png') }}"
                                class="avatar-xl border border-gray-900 rounded-circle mx-auto mt-n7" alt="User Avatar">
                            <h4 class="h3">
                                {{ $viewUser->username }}
                            </h4>
                            <h5 class="fw-normal">{{ $viewUser->getRoleNames()->first() }}</h5>
                            <p class="text-gray mb-4">{{ $viewUser->email }}</p>
